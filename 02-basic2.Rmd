...
1.  import a data set into the program to prepare for data analysis
2.  remove some missing values from the data

## Data Files for This Lab

-   germination.txt [Click here to download the file](https://o365cmu-my.sharepoint.com/:t:/g/personal/pimonrat_t_cmu_ac_th/IQCMvDUji7vLQrxfquxUnGpTAVBV0G42E6KqbVNwpRzQClE?e=mCSDo9)
...
Each row usually represents one observation, and each column is a variable that we measure in experiments.
...
Then we save them in a .txt or .csv file before importing them into R (Figure \@ref(fig:csv)).
...
The easiest way is to format the data into a data frame before importing the data into R.
...
4.  If there is a missing value, put NA or use another symbol such as "?", "." etc. We tell R to recognize the NAs when importing the data.

**Save your file as a .txt or .csv file before importing into R**
...
2.  Check your working directory. Use `dir(getwd())` to see what files are in the current working directory
3.  If the working directory is not set, set it with `setwd()` or Choose Directory in the Session menu
...
```{r eval = FALSE}
# read 1
dat <- read.table("germination.txt", 
header = TRUE, na.string = "NA")
```

What we did is to create an object called "dat" and import the data "germination.txt" into it.
We tell R that we have a header (header = TRUE) and tell R that we have missing values, which are represented by NAs.
...
If you have .csv file we will use **read.csv()** instead of read.table, for example:
...
```{r eval=FALSE}
# see the dimension of data #rows and #columns
dim(dat) 

# see more detailed structure of the data
str(dat) 

# remove every row with missing value
dat.no.na <- na.omit(dat) 
```

The function `na.omit` removes every row with NA.
If you would like to remove a specific row, you may use the command below to create a new data frame without a row in the column 'count' that has 'NA':

```{r eval=FALSE}
## Omit the row with a missing value in the count column
dat.reduced <- dat[!is.na(dat$count), ] 
```
...
Your answers must contain **R code ** that you wrote and **the outputs** you get.
...
Review the file and evaluate whether the data format is appropriate to be imported into R.
Answer in words: What must you change in the data to make the file readable by R?

**2.** In Excel, format the data so the data set is ready to be analyzed in R and save it as an 'example.txt' file.
Your answer should show a screen capture of your corrected file.

**3.**.
In R, create a data frame in R named 'data', and then import the file 'example.txt'.
Show the data frame.

**4.** In R, show the structure of this data frame.

**5.** In R, create a new data frame, called 'data.new' (from 'data') that has no missing values.
...
| 5          | Correct R code and output     | One of the R code or output is not included or incorrect | R code and output are not included or incorrect |
