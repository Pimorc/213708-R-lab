...
Table (\@ref(tab:ancova-tab)) presents the result of ANCOVA that take into the account of initial height. The conclusion is that at least one mean differs from the others. There is also significant effect on initial height on final height of the plants. In comparison, ANCOVA increases statistical power (\@ref(tab:comparison)).
...
| Source of Variation | Sum Square | df | Mean Square (MS) | F | P-value |
|---------------------|----|----|------------------|---|------------|
| Initial Height | `r round(ancova_results$`Sum Sq`[1], digit = 1)` | `r `round(ancova_results$`Df`[1], digit = 1)` | `r round(ancova_results$`Mean Sq`[1], digit = 1)` | `r round(ancova_results$`F value`[1], digit = 1)` | `r round(ancova_results$`Pr(>F)`[1], digit = 6)` |
...
```{r comparison, echo =FALSE, message=FALSE, warning=FALSE, results='as.is'}
comparison <- data.frame(
  Analysis = c("ANOVA", "ANCOVA"),
  F_statistic = c(anova_f, ancova_f),
  P_value = format(c(anova_p, ancova_p), scientific = TRUE),
...
```{r plot-adjusted-means, fig.height=6, fig.cap= "Comparisons of unadjusted and adjusted means" ,}
...
```{r check-slopes, fig.height=6, fig.cap="Homogeneity of Slopes Test"}
...
When ANCOVA shows significant group differences, we use a Tukeys's HSD post-hoc test to identify which groups differ (Figure (\@ref(fig:postdoc-ancova))).

```{r posthoc-ancova, echo=FALSE, results='hide'}
...
```{r plot-posthoc, fig.height=6, fig.cap="Adjusted Means with Post-Hoc Comparisons. Groups with different letters are significantly different"}
...
```{r effect-size-ancova, fig.cap="Partial eta-squared of ANCOVA model of the fertilizer experiment", echo =FALSE, warning=FALSE, message=FALSE}
library(effectsize)
...
<!--chapter:end:Lesson_13_Multiple_Comparisons.Rmd-->
