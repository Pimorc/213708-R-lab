...
$$C_1 = a_{11}X_1 + a_{12}X_2 + \cdots + a_{1n}X_n$$ $$C_2 = a_{21}X_1 + a_{22}X_2 + \cdots + a_{2n}X_n$$ $$\vdots$$
...
The **variance** of variable $A$ is: $$\text{Var}(A) = \frac{\sum (A_i - \bar{A})^2}{N}$$

The **covariance** between variables $A$ and $B$ is: $$\text{Cov}(A, B) = \frac{\sum (A_i - \bar{A})(B_i - \bar{B})}{N}$$
...
```{r pca-concept, echo=FALSE, fig.cap="Conceptual diagram of the first two principal components (PC1 and PC2) fitted to a two-dimensional data cloud. PC1 runs along the axis of greatest variation; PC2 is perpendicular to PC1.", fig.height=4.5, fig.width=4.5, message=FALSE, warning=FALSE, fig.align='center'}
...
```{r screeplot-concept, echo=FALSE, fig.cap="Example scree plot showing eigenvalues for 12 components. The elbow at component 2 (dashed line) suggests retaining the first two principal components. The horizontal dotted line marks eigenvalue = 1 (Kaiser's rule).", fig.height=4, fig.width=6, message=FALSE, warning=FALSE, fig.align="center"}
...
# Preview
row_select_seed <- seq_len(nrow(seed)) %% 10
seed.head <- seed[row_select_seed == 1, -c(1:2,13)] # select some rows
seed.for.table <- seed.head %>%
  relocate(species)
rownames(seed.for.table) <- NULL
...
seed.for.table %>% 
  kbl(align = "c",
      ...
      caption = "Some sample data of seed morphology data set") %>%
...
```{r boxplots-seed, echo=FALSE, fig.cap="Boxplots of all eight seed morphological traits by species. Note that Garcinia cowa seeds are substantially larger than the other two species on all traits.", fig.height=7, fig.width=10, message=FALSE, warning=FALSE, fig.align = "center"}
...
```{r run-pca, echo=FALSE, message=FALSE, warning=FALSE, include = FALSE, fig.align='center'}
...
We test univariate normality of each trait within each species using the Shapiro-Wilk test. A non-significant result (p \> 0.05) is consistent with normality. In our cases, the normalliuty test was satisfied except for two variables each of *E. subumbrans* and *G. cowa*.

```{r normality-test-lda, echo=FALSE, message=FALSE, warning=FALSE, results='asis', include = FALSE}
library(dplyr)
library(kableExtra)
...
```{r collinearity, echo=FALSE, fig.cap="Correlation matrix of seed morphological traits. Darker blue = strong positive correlation; white = no correlation. Highly correlated pairs (e.g., volume and dry weight) carry redundant information.", fig.height=5, fig.width=6, message=FALSE, warning=FALSE, fig.align='center'}
...
5.  What test was used to evaluate the assumption of the homogeneity of covariance matrices in the seed data example? If this assumption is violated, what alternative method could be used?
