...
```{r plant-data-tab, echo=FALSE, message=FALSE, warning=FALSE, tab.cap="The data of fertilizer experiment for ANOVA"}
...
```{r summary-data, echo=FALSE, message=FALSE, warning=FALSE, tab.cap="Summary of experiment data for ANOVA"}
summary_plant%>%
  kbl(align = "c",
...
In our case, the result of the Levene's Test (P-value  = `r round(levene$"Pr(>F)"[1], digit = 2)`) indicates that the null hypothesis is not rejected. The variances among groups are homogeneous (\@ref(tab:levene-tab)).
...
```{r tukey-hsd-tab, message=FALSE, echo = FALSE, tab.cap= "Tukey's HSD results comparing three treatment pairs", warning=FALSE} 
...
```{r dunnett-tab, message=FALSE, echo = FALSE, tab.cap= "Dunnett's results comparing each treatments with the control treatment", warning=FALSE} 
...
> "An analysis of variance (ANOVA) was conducted to evaluate the effect of fertilizer treatment (Control, Fertilizer A, and B) on final plant height. There was a significant effect of fertilizer treatment on final plant height (F(2, 18) = `r anova_results$"F value"[1]`, P < 0.001, η² = `r round(sse/ss_total, digit = 2)`). 
>
...
Researchers want to compare three fertilizer treatments (Organic, Synthetic, and Control - no fertilizer) on final plant height (\@ref(tab:ancova-data-tab)). However, seedlings have different initial sizes when the experiment begins, which could confound our results.
...
```{r ancova-data-tab, echo=FALSE, message=FALSE, warning=FALSE, tab.cap="The data of fertilizer experiment"}
### Write the data into a table
library(knitr)
library(kableExtra)
...
```{r summary-data-ancova, echo=FALSE, message=FALSE, warning=FALSE, tab.cap="Summary of fertilizer experiment data by treatment"}
summary_stats %>%
  kbl(align = "c",
...
```{r anova-only-tab, message=FALSE, echo = FALSE, include=FALSE, tab.cap= "ANOVA results that ignore initial height", warning=FALSE} 
...
```{r comparison, echo =FALSE, message=FALSE, warning=FALSE, results='as.is', tab.cap="Comparisons fo F statistics and P-value of ANOVA and ANCOVA", include=FALSE}
...
THe researchers should check for:
-   **Normality of residuals**
