...
From the fertilizer experiment, the mean of plant height from Fertilizer A did not significantly from that of the control group. Other pair of comparison shows significantly differences (\@ref(tab:tukey-hsd-tab)).

```{r tukey-hsd, message=FALSE, echo = FALSE, results=FALSE, warning=FALSE}
...
```{r tukey-hsd-tab, message=FALSE, echo = FALSE, results='as.is', fig.cap= "Tukey's HSD results comparing three treatment pairs", warning=FALSE} 
kbl(tukey_table, digit = 2, row.names = FALSE) %>%
kable_styling(bootstrap_options = "striped", full_width = F)
```

The results are visualized with letters to indicate the significant difference (Figre (\@ref(fig:visualize-hsd))). The treatments that share the same letters are not significantly different from each other.

```{r visualize-hsd, message=FALSE, warning=FALSE, fig.height = 5, fig.cap= Visulizeation of Tukey's HSD results}
...
```{r dunnett-tab, message=FALSE, echo = FALSE, results='as.is', fig.cap= "Dunnett's results comparing each treatments with the control treatment", warning=FALSE} 
kbl(dunnett_results$`No fertilizer`, digit = 2, row.names = TRUE) %>%
kable_styling(bootstrap_options = "striped", full_width = F)
```

## Reporting ANOVA Results
The following paragraph presents the report from ANOVA.

> "An analysis of variance (ANOVA) was conducted to evaluate the effect of fertilizer treatment (Control, Fertilizer A, and B) on final plant height. There was a significant effect of fertilizer treatment on final plant height (F(2, 18) = `r round(anova_results$`F value`[1], P < 0.001, η² = `r round(sse/ss_total, digit = 2)`). 
>
...
```{r ancova-data-tab, echo=FALSE, message=FALSE, warning=FALSE}
### Write the data into a table
library(knitr)
library(kableExtra)
...
First, let's explore the data before testing. The research should make the summary of the data separated by the factor of interest (Fertilizer) in Table .

```{r summary-data-ancova, echo=FALSE, message=FALSE, warning=FALSE}
summary_stats %>%
  kbl(align = "c",
...
Table (\@ref(tab:anova-only-tb)) presents the result of ANOVA that did not take into the account of initial height. The conclusion is that at least one mean differs from the others.
...
```{r anova-only-tab, message=FALSE, echo = FALSE, include=FALSE, fig.cap= "ANOVA results that ignore initial height, warning=FALSE} 
...
| Source of Variation | Sum Square | df | Mean Square (MS) | F | P-value |
|---------------------|----|----|------------------|---|------------|
| Treatment (Between) | `r round(anova_results_cov$`Sum Sq`[1], digit = 1)` | `r `round(anova_results_cov$`Df`[1], digit = 1)` | `r round(anova_results_cov$`Mean Sq`[1], digit = 1)` | `r round(anova_results_cov$`F value`[1], digit = 1)` | `r round(anova_results_cov$`Pr(>F)`[1], digit = 6) ` |
| Error (Within) | `r `round(anova_results-cov$`Sum Sq`[2], digit = 1)` | `r round(anova_results_cov$`Df`[2], digit = 1)` | `r round(anova_results_cov$`Mean Sq`[2], digit = 1)` |  |  |
...
Table (\@ref(tab:ancova-tab)) presents the result of ANCOVA that take into the account of initial height. The conclusion is that at least one mean differs from the others. There is also significant effect on initial height on final height of the plants.
...
| Source of Variation | Sum Square | df | Mean Square (MS) | F | P-value |
|---------------------|----|----|------------------|---|------------|
| Initial Height | `r round(ancova_results$`Sum Sq`[1], digit = 1)` | `r `round(ancova_results$`Df`[1], digit = 1)` | `r round(ancova_results$`Mean Sq`[1], digit = 1)` | `r round(ancova_results$`F value`[1], digit = 1)` | `r round(anova_results$`Pr(>F)`[1], digit = 6) ` |
| Treatment (Between Fertilizer) | `r `round(ancova_results$`Sum Sq`[2], digit = 1)` | `r round(ancova_results$`Df`[2], digit = 1)` | `r round(ancova_results$`Mean Sq`[2], digit = 1)` |  |  |
...
In comparison, ANCOVA increases statistical power.
